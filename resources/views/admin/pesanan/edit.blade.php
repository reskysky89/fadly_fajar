<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-bold text-xl text-gray-800 dark:text-white leading-tight">
            Proses Pesanan: {{ $transaksi->id_transaksi }}
        </h2>
    </x-slot>
    ...
    @php
        $initialItems = $transaksi->details->map(function($item) {
            ...
            // 3. Pastikan satuan tersimpan ada di opsi
    ...
    <div class="h-[calc(100vh-65px)] flex flex-col bg-gray-100 dark:bg-gray-900" 
            ...
            <div class="grid grid-cols-12 gap-4">
                <div class="col-span-9 grid grid-cols-3 gap-4">
                    ...
                    <div class="col-span-2">
                        <label class="block text-xs text-gray-500 uppercase font-bold">Keterangan / Alamat</label>
                        <div class="text-sm text-gray-600 truncate" title="{{ $transaksi->keterangan }}">{{ $transaksi->keterangan }}</div>
                    </div>
                </div>
                <div class="col-span-3 bg-blue-600 text-white font-mono font-bold flex flex-col justify-center items-end px-4 rounded shadow-inner">
                    <span class="text-xs text-blue-200 uppercase tracking-widest">Total Tagihan</span>
                    {{-- ANTI NaN: Pakai fungsi formatRupiah yang aman --}}
        ...
        <div class="bg-gray-200 dark:bg-gray-800 p-3 border-t border-gray-300 dark:border-gray-700 flex-shrink-0 z-20">
            <div class="flex justify-between items-center">
                <div class="text-sm text-gray-500">
                    Shortcut: <b>[F2]</b> Baris Baru, <b>[END]</b> Bayar & Selesai, <b>[DEL]</b> Hapus
                </div>
                <div class="flex space-x-2">
                    <a href="{{ route('pesanan.index') }}" class="px-6 py-3 bg-gray-500 hover:bg-gray-600 text-white font-bold rounded shadow">BATAL</a>
        ...
        <div x-show="modalBayarOpen" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center" x-transition x-cloak>
            <div class="bg-white w-full max-w-md p-6 rounded-lg shadow-2xl transform scale-100 transition-transform">
                <h2 class="text-2xl font-bold mb-4 text-gray-800 border-b pb-2">Finalisasi Pesanan</h2>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" x-model.number="uangDiterima" id="input-bayar" 
                               ...
                               @keydown.enter.prevent="prosesSelesai()" 
                               @keydown.escape="modalBayarOpen = false" 
                               class="w-full text-right text-3xl font-bold border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 p-2 text-green-700" 
                               placeholder="0">
                    </div>
                ...
                <div class="mt-6 flex justify-end space-x-2">
                    ...
                    <button @click="prosesSelesai()" class="px-6 py-2 bg-green-600 text-white font-bold rounded shadow hover:bg-green-700">
                        PROSES SELESAI
                    </button>
                </div>
            </div>
    ...
    <script>
        function prosesPesanan() {
            return {
                ...
                init() {
                    // Selalu pastikan ada minimal 1 baris kosong jika data kosong
                    if (this.items.length === 0) this.tambahBarisBaru();
                    
                    // Reorder opsi satuan (Opsional, jika mau yang dipilih naik ke atas)
                    this.items.forEach(item => this.ensureReorderAfterFill(item));
                },

                // --- 1. PERBAIKAN LOGIKA HITUNG AGAR TIDAK NaN ---
                hitungSubtotal(index) {
                    const item = this.items[index];
                    let qty = parseFloat(item.qty) || 0;   // Jika kosong, jadi 0
                    let harga = parseFloat(item.harga) || 0; // Jika kosong, jadi 0
                ...
                handleGlobalKey(e) {
                    ...
                    if (this.modalBayarOpen) { 
                        if (e.key === 'Enter' && !this.isProcessing) { e.preventDefault(); this.prosesSelesai(); } 
                    ...
                    if (e.key === 'Escape') { window.location.href = "{{ route('pesanan.index') }}"; }
                },
                ...
                async prosesSelesai() {
                    ...
                    if (this.uangDiterima < this.grandTotal) { alert('Uang pembayaran kurang!'); return; }
                    
                    this.isProcessing = true;
                    
                    // --- PERBAIKAN: FILTER BARIS KOSONG ---
                    // Hanya kirim item yang punya ID Produk (yang valid)
                    const validItems = this.items.filter(item => item.id_produk_final && item.id_produk_final !== '');
                    
                    if (validItems.length === 0) {
                         alert('Tidak ada barang yang valid untuk diproses!');
                    ...
                    try {
                        const urlUpdate = "{{ route('pesanan.update', $transaksi->id_transaksi) }}";
