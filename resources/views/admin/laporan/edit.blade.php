<x-app-layout>
    ...
    @php
        $initialItems = $transaksi->details->map(function($item) {
            ...
            // 3. Pastikan satuan yang tersimpan ada di opsi
            $satuanTersimpan = $item->satuan;
            $isFound = false;
            foreach ($opsiSatuan as $opt) {
                if ($opt['nama'] == $satuanTersimpan) {
                    $isFound = true;
                    break;
                }
            }
    ...
    <div class="h-[calc(100vh-140px)] flex flex-col" 
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 relative">
                        <thead class="bg-gray-200 dark:bg-gray-700 sticky top-0 z-10 shadow-sm">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase w-32">Satuan</th>
                                <th class="px-4 py-3 text-right text-xs font-bold text-gray-700 uppercase w-40">Harga</th>
                                <th class="px-4 py-3 text-right text-xs font-bold text-gray-700 uppercase w-48">Subtotal</th>
                            ...
                            <template x-for="(item, index) in items" :key="item.id_temp">
                                <tr :class="activeRow === index ? 'bg-blue-100 dark:bg-blue-900' : 'hover:bg-gray-50 dark:hover:bg-gray-700'" 
                                    ...
                                    <td class="px-4 py-2">
                                        ...
                                        <p x-show="item.error" x-text="item.error" class="text-xs text-red-500 mt-1 font-bold"></p>
                                    </td>

                                    {{-- NAMA BARANG --}}
                                    <td class="px-4 py-2">
                                        <input type="text" :value="item.nama_barang" readonly @focus="activeRow = index" class="w-full bg-transparent border-transparent rounded-md text-gray-600 text-sm cursor-not-allowed focus:ring-0 h-9">
                                    </td>

                                    {{-- QTY --}}
                                    <td class="px-4 py-2">
                                        <input type="text" inputmode="numeric" x-model="item.qty" :id="'qty_' + index"
                                    ...
                                    <td class="px-4 py-2">
                                        <input type="number" x-model.number="item.harga" :id="'harga_' + index"
                                               @focus="activeRow = index; $el.select()" @input="hitungSubtotal(index)"
                                               @keydown.delete="hapusBaris(index)"
                                               @keydown.enter.prevent="fokusBawah(index, 'kode')"
                                               ...
                                               class="w-full text-right border-gray-300 dark:border-gray-600 rounded-md shadow-sm font-mono focus:ring-blue-500 focus:border-blue-500 h-9"
                                               :disabled="!item.id_produk_final">
                                    </td>
                                    ...
                                    <td class="px-4 py-2 text-center">
                                        <button type="button" :id="'hapus_' + index" @click="hapusBaris(index)" 
                                                ...
                                                x-show="items.length > 1" class="text-gray-400 hover:text-red-600 transition-colors focus:outline-none focus:text-red-600 focus:ring-2 focus:ring-red-500 rounded" tabindex="0">
        ...
        <div class="bg-gray-200 dark:bg-gray-800 p-3 border-t border-gray-300 dark:border-gray-700 flex-shrink-0 z-20">
            <div class="flex justify-between items-center">
                <div class="text-sm text-gray-500">
                     <button @click="tambahBarisBaru()" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-bold rounded shadow mr-4 text-xs">+ Baris Baru [F2]</button>
                     Shortcut: <b>[F2]</b> Tambah, <b>[END]</b> Bayar, <b>[DEL]</b> Hapus
                </div>
                ...
                <div class="flex space-x-2">
                    ...
                    {{-- 2. SIMPAN (Update Saja) --}}
                    <button @click="prosesBayar(false)" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm rounded shadow flex items-center gap-2">
                        SIMPAN
                    </button>

                    {{-- 3. SIMPAN & CETAK (Hijau) --}}
                    <button @click="bukaModalBayar()" class="px-8 py-3 bg-green-600 hover:bg-green-700 text-white font-bold text-xl rounded shadow-lg flex items-center gap-2">
                        SIMPAN & CETAK [END]
                    </button>
                </div>
            </div>
        ...
        {{-- MODAL BAYAR (Untuk Simpan & Cetak) --}}
        <div x-show="modalBayarOpen" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center" x-transition x-cloak><div class="bg-white dark:bg-gray-800 w-full max-w-md p-6 rounded-lg shadow-2xl transform scale-100 transition-transform"><h2 class="text-2xl font-bold mb-4 text-gray-800 dark:text-white border-b pb-2">Update & Cetak</h2><div class="space-y-4"><div class="flex justify-between text-xl"><span>Total Baru</span><span class="font-bold" x-text="formatRupiah(grandTotal)"></span></div><div><label class="block text-sm font-bold text-gray-700 mb-1">Uang Diterima (Update)</label><input type="number" x-model.number="uangDiterima" id="input-bayar" @input="hitungKembalian()" @keydown.enter.prevent="prosesBayar(true)" @keydown.escape="modalBayarOpen = false" class="w-full text-right text-3xl font-bold border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2" placeholder="0"></div><div class="flex justify-between text-xl pt-2 border-t text-blue-600"><span>Kembalian</span><span class="font-bold" x-text="formatRupiah(kembalian)"></span></div></div><div class="mt-6 flex justify-end space-x-2"><button @click="modalBayarOpen = false" class="px-4 py-2 bg-gray-500 text-white rounded">Batal</button><button @click="prosesBayar(true)" class="px-6 py-2 bg-green-600 text-white font-bold rounded">CETAK & SIMPAN</button></div></div></div>
    ...
    <script>
        function transaksiEdit() {
            return {
                ...
                barisYangSedangDiisi: null, uangDiterima: {{ $transaksi->bayar }}, kembalian: {{ $transaksi->kembalian }}, searchQueryModal: '', isProcessing: false,

                init() {
                    if(this.items.length === 0) this.tambahBarisBaru();

                    // LOGIKA REORDER SATUAN (PENTING)
                    this.items.forEach((item, idx) => {
                        this.ensureReorderAfterFill(item);
                ...
                get grandTotal() { return this.items.reduce((sum, item) => sum + (item.subtotal || 0), 0); },
                ...
                async prosesBayar(cetakStruk) {
                    ...
                    try {
                        ...
                        if (result.success) { 
                            let msg = 'Transaksi Berhasil Diupdate!';
                            if (cetakStruk) {
                                // Buka tab baru ke halaman cetak
                                let urlStruk = "{{ route('kasir.transaksi.cetak') }}?id=" + encodeURIComponent("{{ $transaksi->id_transaksi }}");
                ...
                tambahBarisBaru() { 
                    this.items.push({ id_temp: Date.now() + Math.random(), kode_item: '', id_produk_final: '', nama_barang: '', qty: 1, id_satuan: '', harga: 0, subtotal: 0, opsi_satuan: [], error: '' }); 
                    this.activeRow = this.items.length - 1; 
                    this.$nextTick(() => { document.getElementById('kode_' + this.activeRow)?.focus(); }); 
                },
                
                hapusBaris(index) { 
                    const item = this.items[index]; 
                    if (!item.nama_barang || item.nama_barang === '') { 
                        this.items.splice(index, 1); 
                        if (this.items.length === 0) this.tambahBarisBaru(); 
                        this.$nextTick(() => document.getElementById('kode_' + Math.min(index, this.items.length-1))?.focus()); 
                        return; 
                    } 
                    this.items.splice(index, 1); 
                    if (this.items.length === 0) this.tambahBarisBaru(); 
                ...
                async scanProduk(index) { 
                    ...
                    const item = this.items[index]; 
                    const kode = item.kode_item; 
                    if (kode.length < 2) return; 
                    if (item.id_produk_final) return; 
                    try { 
                        const response = await fetch(`{{ route('kasir.transaksi.cariProduk') }}?search=${kode}`); 
                        const data = await response.json(); 
                        if (data.length === 0) { item.error = 'Produk tidak ditemukan!'; item.nama_barang = ''; } 
                            ...
                            this.$nextTick(() => { document.getElementById('modal-search-input').focus(); document.getElementById('modal-search-input').select(); }); 
                        } 
                ...
                isiBaris(index, semuaOpsi, produkTerpilih) { 
                    const item = this.items[index]; 
                    item.id_produk_final = produkTerpilih.id_produk; 
                    item.kode_item = produkTerpilih.id_produk; 
                    item.nama_barang = produkTerpilih.nama_produk; 
                    item.opsi_satuan = semuaOpsi.filter(p => p.id_produk === produkTerpilih.id_produk).map(p => ({ nama: p.nama_satuan, harga: p.harga_jual, stok: p.stok_real })); 
                    item.satuan = produkTerpilih.nama_satuan; 
                    item.harga = produkTerpilih.harga_jual; 
                    item.error = ''; 
                    this.ensureReorderAfterFill(item); 
                    this.hitungSubtotal(index); 
                    this.$nextTick(() => { document.getElementById('qty_' + index)?.focus(); }); 
                    if (index === this.items.length - 1) { this.tambahBarisBaru(); } 
                },
                ...
                fokusBawah(index, colName) { if (this.modalPencarianOpen) return; const nextIndex = index + 1; if (nextIndex < this.items.length) { this.activeRow = nextIndex; document.getElementById(colName + '_' + nextIndex)?.focus(); } else { this.tambahBarisBaru(); } },
