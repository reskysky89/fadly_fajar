<x-app-layout>
    ...
    <div class="h-[calc(100vh-140px)] flex flex-col" x-data="transaksiKasir()">
                                ...
                                <tr class="hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td class="px-4 py-2">
                                        <input type="text" 
                                               ...
                                               class="w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 font-mono text-sm font-bold text-blue-700 uppercase"
                                               placeholder="Scan..." 
                                               autofocus>
                                    ...
                                    <td class="px-4 py-2">
                                        <input type="text" x-model="baris.nama_barang" readonly 
                                               class="w-full bg-gray-50 border-transparent rounded-md text-gray-600 text-sm cursor-not-allowed focus:ring-0">
                                    </td>

                                    {{-- QTY --}}
                                    <td class="px-4 py-2">
                                        <input type="number" x-model.number="baris.qty" 
                                               :id="'qty_' + index"
                                               ...
                                               class="w-full text-center border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 font-bold">
                                    </td>

                                    {{-- SATUAN (Dropdown) --}}
                                    <td class="px-4 py-2">
                                        <select x-model="baris.id_satuan" 
                                                :id="'satuan_' + index"
                                                ...
                                                class="w-full text-sm border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                            <template x-for="opsi in baris.opsi_satuan" :key="opsi.id">
                                                {{-- Tampilkan Nama Satuan (PCS/DUS) --}}
                                    ...
                                    <td class="px-4 py-2 text-right">
                                        <input type="text" :value="formatRupiah(baris.harga)" readonly 
                                               class="w-full bg-gray-50 border-transparent rounded-md text-gray-600 text-right text-sm cursor-not-allowed focus:ring-0 font-mono">
                                    </td>

                                    {{-- TOTAL --}}
                                    <td class="px-4 py-2 text-right">
                                        <span x-text="formatRupiah(baris.subtotal)" class="font-bold font-mono text-gray-800 text-lg"></span>
                                    </td>

                                    {{-- HAPUS --}}
                                    <td class="px-4 py-2 text-center">
                                        <button type="button" @click="hapusBaris(index)" x-show="barisTabel.length > 1" class="text-red-500 hover:text-red-700 font-bold" tabindex="-1">&times;</button>
                                    </td>
                                </tr>
                            </template>
                        </tbody>
        ...
        {{-- BAGIAN 3: FOOTER (Bawah) --}}
        <div class="bg-gray-200 dark:bg-gray-800 p-4 border-t border-gray-300 dark:border-gray-700 flex-shrink-0">
            <div class="flex justify-between items-end">
                <div class="text-sm text-gray-500">
                    Shortcut: <span class="font-bold">[F2]</span> Scan, <span class="font-bold">[END]</span> Bayar
                </div>
                <div class="flex space-x-2">
                    <button @click="resetForm()" class="px-6 py-3 bg-red-500 hover:bg-red-600 text-white font-bold rounded shadow">Batal [ESC]</button>
        ...
        <div x-show="modalBayarOpen" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center" x-transition x-cloak>
            <div class="bg-white dark:bg-gray-800 w-full max-w-md p-6 rounded-lg shadow-2xl transform scale-100 transition-transform">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" x-model.number="uangDiterima" id="input-bayar"
                               @input="hitungKembalian()" @keydown.enter.prevent="prosesBayar()" @keydown.escape="modalBayarOpen = false"
                               class="w-full text-right text-3xl font-bold border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2" placeholder="0">
                    </div>
                ...
                <div class="mt-6 flex justify-end space-x-2">
                    ...
                    <button @click="prosesBayar()" :disabled="uangDiterima < grandTotal" 
                            :class="uangDiterima < grandTotal ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'"
                            class="px-6 py-2 bg-blue-600 text-white font-bold rounded">
                        PROSES (Enter)
                    </button>
                </div>
            </div>
        ...
        {{-- =================================================== --}}
        {{-- MODAL PENCARIAN PRODUK (Pop-up Daftar Item) --}}
        {{-- =================================================== --}}
        <div x-show="modalPencarianOpen" class="fixed inset-0 bg-gray-600 bg-opacity-75 z-50 flex items-center justify-center" x-transition x-cloak>
            <div class="relative mx-auto p-6 border w-full max-w-3xl shadow-lg rounded-md bg-white dark:bg-gray-800 max-h-[80vh] flex flex-col">
                <h3 class="text-xl font-bold mb-4 border-b pb-2">Pilih Item</h3>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <template x-for="produk in searchResultsModal" :key="produk.unique_id">
                                <tr class="hover:bg-blue-50 dark:hover:bg-gray-700 cursor-pointer" @click="pilihProdukDariModal(produk)">
                                    <td class="px-4 py-3">
                                        <div class="font-bold" x-text="produk.nama_produk"></div>
                                        <div class="text-xs text-gray-500" x-text="produk.id_produk"></div>
                                    </td>
                                    <td class="px-4 py-3 text-center font-bold">
                                        <span class="px-2 py-1 bg-gray-200 rounded text-xs" x-text="produk.nama_satuan"></span>
                                    </td>
                                    <td class="px-4 py-3 text-right font-mono text-blue-600 font-bold" x-text="formatRupiah(produk.harga_jual)"></td>
                                    <td class="px-4 py-3 text-center" x-text="produk.stok_real"></td>
                                    <td class="px-4 py-3 text-center">
                                        <button class="px-3 py-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-700">Pilih</button>
                                    </td>
                                </tr>
                            </template>
                        </tbody>
                ...
                <div class="mt-4 flex justify-end pt-2 border-t">
                    <button @click="modalPencarianOpen = false" class="px-4 py-2 bg-gray-500 text-white rounded">Tutup [ESC]</button>
                </div>
            </div>
        </div>
            ...
            return {
                ...
                init() {
                    ...
                    window.addEventListener('keydown', (e) => {
                        ...
                        if (e.key === 'Escape' && this.modalPencarianOpen) { this.modalPencarianOpen = false; }
                    });
                },
                ...
                tambahBarisBaru() {
                    this.barisTabel.push({
                        ...
                        kode_item: '',
                        id_produk_final: '', // ID Produk asli untuk backend
                        nama_barang: '',
                        qty: 1,
                        id_satuan: '',
                        harga: 0,
                        subtotal: 0,
                        opsi_satuan: [], // Daftar opsi satuan (PCS/DUS)
                        error: ''
                    });
                    this.$nextTick(() => {
                        const index = this.barisTabel.length - 1;
                        document.getElementById('kode_' + index)?.focus();
                    });
                },
                hapusBaris(index) {
                ...
                async scanProduk(index) {
                    ...
                    // Jika modal sedang buka, jangan scan lagi
                    if (this.modalPencarianOpen) return;

                    // Jika produk sudah dipilih (ID Final ada), jangan cari lagi
                    ...
                    try {
                        ...
                        if (data.length === 0) {
                            ...
                            // SKENARIO A: Cuma 1 hasil (Langsung Masuk)
                            this.isiBaris(index, data, data[0]);
                        } else {
                            // SKENARIO B: Banyak hasil (Buka Modal)
                ...
                isiBaris(index, semuaOpsi, produkTerpilih) {
                    ...
                    // Auto tambah baris baru jika di akhir
                    if (index === this.barisTabel.length - 1) {
                        this.tambahBarisBaru();
                    }
                },

                // Dipanggil saat user klik "Pilih" di Modal
                ...
                hitungSubtotal(index) {
                    const baris = this.barisTabel[index];
                    baris.subtotal = baris.qty * baris.harga;
                },

                get grandTotal() {
                    return this.barisTabel.reduce((sum, baris) => sum + baris.subtotal, 0);
                },

                // --- NAVIGASI KEYBOARD ---
                fokusBawah(index, colName) {
                    const nextIndex = index + 1;
                    if (nextIndex < this.barisTabel.length) {
                        document.getElementById(colName + '_' + nextIndex)?.focus();
                    } else {
                        this.tambahBarisBaru();
                    }
                },
                fokusAtas(index, colName) {
                    if (index > 0) document.getElementById(colName + '_' + (index - 1))?.focus();
                },
                ...
                async prosesBayar() {
                    ...
                    // 2. Siapkan Data
                    // Kita TIDAK kirim ID Transaksi dari sini, biar Server yang generate agar aman
                    const payload = {
                        total_harga: this.grandTotal,
                        bayar: this.uangDiterima,
                        kembalian: this.kembalian,
                        // Filter baris kosong & ambil data penting saja
