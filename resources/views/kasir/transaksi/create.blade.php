<x-app-layout>
    ...
    <div class="h-[calc(100vh-140px)] flex flex-col" x-data="transaksiKasir()">
        ...
        {{-- BAGIAN 2: TABEL TRANSAKSI (Scrollable) --}}
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 relative">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                            <template x-for="(baris, index) in barisTabel" :key="baris.id_temp">
                                {{-- 
                                    PERBAIKAN 1: Dynamic Class untuk Highlight Baris Aktif 
                                    Jika activeRow == index, warna jadi biru muda (bg-blue-100)
                                --}}
                                <tr :class="activeRow === index ? 'bg-blue-100 dark:bg-blue-900' : 'hover:bg-gray-50 dark:hover:bg-gray-700'" 
                                    ...
                                    <td class="px-4 py-2">
                                        {{-- 
                                            PERBAIKAN 2: 
                                            - @focus="activeRow = index" agar highlight pindah otomatis 
                                            - @keydown.delete="hapusBaris(index)" agar tombol Delete menghapus baris
                                        --}}
                                        <input type="text" 
        ...
        <div class="bg-gray-200 dark:bg-gray-800 p-4 border-t border-gray-300 dark:border-gray-700 flex-shrink-0">
            <div class="flex justify-between items-end">
                ...
                <div class="flex space-x-2">
                    <button @click="resetForm()" class="px-6 py-3 bg-red-500 hover:bg-red-600 text-white font-bold rounded shadow flex items-center gap-2">
                        BATAL [ESC]
                    </button>
        ...
        {{-- MODAL PEMBAYARAN & PENCARIAN (Sama seperti sebelumnya) --}}
        {{-- Modal Bayar --}}
        <div x-show="modalBayarOpen" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center" x-transition x-cloak>
            <div class="bg-white dark:bg-gray-800 w-full max-w-md p-6 rounded-lg shadow-2xl transform scale-100 transition-transform">
                ...
                <div class="mt-6 flex justify-end space-x-2">
                    ...
                    <button @click="prosesBayar()" :disabled="uangDiterima < grandTotal" class="px-6 py-2 bg-blue-600 text-white font-bold rounded" :class="uangDiterima < grandTotal ? 'opacity-50' : ''">PROSES (Enter)</button>
                </div>
            </div>
        </div>
    ...
    <script>
        function transaksiKasir() {
            return {
                barisTabel: [],
                activeRow: 0, // Melacak baris mana yang aktif (biru)
                ...
                init() {
                    ...
                    window.addEventListener('keydown', (e) => {
                        ...
                        if (e.key === 'Escape') { this.modalBayarOpen = false; this.modalPencarianOpen = false; }
                ...
                tambahBarisBaru() {
                    this.barisTabel.push({
                        ...
                        kode_item: '', id_produk_final: '', nama_barang: '', qty: 1, id_satuan: '', harga: 0, subtotal: 0, opsi_satuan: [], error: ''
                    });
                ...
                hapusBaris(index) {
                    // Jika input kosong atau user menekan Delete/Backspace saat field kosong -> Hapus baris
                    // Disini kita buat agar Delete key selalu menghapus baris
                    this.barisTabel.splice(index, 1);
                    if (this.barisTabel.length === 0) this.tambahBarisBaru();
                    
                    // Adjust active row
                    if (this.activeRow >= this.barisTabel.length) {
                        this.activeRow = this.barisTabel.length - 1;
                    }
                    // Fokus balik ke baris yang aktif
                    this.$nextTick(() => {
                        document.getElementById('kode_' + this.activeRow)?.focus();
                    });
                },

                // ... (Fungsi scanProduk, isiBaris, pilihProdukDariModal, updateHarga, hitungSubtotal, grandTotal SAMA) ...
                // Agar kode tidak kepanjangan, bagian logika AJAX dan hitungan sama persis dengan sebelumnya
                ...
                get grandTotal() { return this.barisTabel.reduce((sum, item) => sum + item.subtotal, 0); },

                // --- NAVIGASI KEYBOARD UPDATE ---
                fokusBawah(index, colName) {
                    const nextIndex = index + 1;
                    if (nextIndex < this.barisTabel.length) {
                        this.activeRow = nextIndex; // Update highlight
                ...
                fokusAtas(index, colName) {
                    if (index > 0) {
                        this.activeRow = index - 1; // Update highlight
                        document.getElementById(colName + '_' + (index - 1))?.focus();
                    }
                },
